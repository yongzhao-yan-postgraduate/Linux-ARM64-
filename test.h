...
MY_STATIC void test1(void)
{
	...
	if (pFile)
	{
		phy_addr = get_pagemap_phy_addr(pFile, 0x6fff174000);
		printk(KERN_INFO "pagemap phy_addr 0x%llx\n", phy_addr);

		char buf[4];
		memset(buf, 0, 4);
		if (read_ram_physical_addr_to_kernel(phy_addr, buf, 4))
		{
			int i;
			for (i = 0; i < 4; i++)
			{
...
MY_STATIC void test2(void)
{
	...
	get_proc_maps_list_to_kernel(proc_pid_struct, 4096, &test,sizeof(test), NULL);
...
MY_STATIC void test3(void)
{
	...
	if (proc_pid_struct)
	{
		size_t phy_addr = get_proc_phy_addr(proc_pid_struct, 0x6fff174000);
